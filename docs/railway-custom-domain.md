...
Этот чек-лист поможет перенести фронтенд на домен `splash.nboard.au` (или любой другой поддомен).
...
3. Введите `splash.nboard.au` и нажмите **Add**. Railway сразу покажет, какие DNS-записи нужно создать.
...
1. **Если используете поддомен (рекомендуется):**
   - Создайте CNAME-запись для `splash`.
   - Значение укажите таким же, как подсказал Railway, обычно это `splash-frontend-production.up.railway.app` (название зависит от сервиса).
...
Railway для фронтенда использует команду `npm run preview`, а Vite по умолчанию пропускает только заранее перечисленные хосты. Из-за этого при открытии `https://splash.nboard.au` появляется сообщение вида:

```
Blocked request. This host ("splash.nboard.au") is not allowed. To allow this host, add "splash.nboard.au" to preview.allowedHosts in vite.config.js.
```
...
3. В поле **Key** введите `VITE_PREVIEW_ALLOWED_HOSTS`, а в **Value** — `splash.nboard.au`.
4. Сохраните переменную и перезапустите деплой (**Deployments → Restart latest**).

> ℹ️ Можно указать несколько доменов через запятую, например: `splash.nboard.au,recruitment.nboard.au,www.recruitment.nboard.au`.
...
2. Проверьте значение `VITE_API_URL` — оно должно указывать на актуальный домен бэкенда, например `https://splash-backend-production.up.railway.app`.
...
5. Если переменную забыли указать, приложение попытается вычислить домен бэкенда автоматически (например, `splash-backend-production.up.railway.app`). Однако это эвристика, поэтому всегда задавайте `VITE_API_URL` явно, чтобы исключить ошибки в будущем.
...
1. Откройте `https://splash.nboard.au` (фронтенд) и `https://recruitment.nboard.au` (бэкенд, если подключали) в браузере с чистым кэшем или в режиме инкогнито.
...
1. Можно использовать **один и тот же Resend API key** для нескольких бэкендов Railway. Главное — чтобы каждый сервис
   передавал валидный `RESEND_FROM`.
2. Если `RESEND_FROM` использует адрес на новом домене (например, `login@splash.nboard.au`), убедитесь, что этот домен
   отображается в Resend со статусом **Verified**. Для дополнительных поддоменов (например, `recruitment2.0.nboard.au`)
   добавьте и подтвердите DNS-записи отдельно.
3. После смены отправителя обновите переменную `RESEND_FROM` в бэкенде Railway и перезапустите деплой, чтобы приложение
   использовало новый адрес. Для сервисов, которые делят API key, `RESEND_FROM` может быть одинаковым (например,
   `login@nboard.au`), если этот домен уже верифицирован.
4. Пока домен не подтверждён, Resend вернёт ошибку 400/403 с упоминанием домена, а API бэкенда ответит HTTP 424 с подсказкой
   «Sender domain is not verified». После верификации повторный запрос кода пройдёт успешно.

## Нужно ли менять код?

- **Frontend.** Код менять не требуется: Railway автоматически проксирует домен на тот же деплой. Важно лишь, чтобы переменные окружения, которые формируют ссылки (например, `VITE_API_URL`), были заданы с учётом нового домена бэкенда.
...
- **Инвайты и письма.** Если в письмах или конфигурациях указывается адрес фронтенда, обновите значение (например, `INVITE_URL=https://splash.nboard.au/login`). Это можно сделать через вкладку **Variables** в Railway без изменения исходников.

## Частые вопросы

- **Нужно ли покупать SSL-сертификат?** Нет, Railway выпустит Let's Encrypt бесплатно.
- **Что делать, если Railway не видит CNAME?** Подождите 10–15 минут и проверьте DNS через `nslookup splash.nboard.au`. Если всё ещё указывает на старое значение, убедитесь, что на стороне регистратора запись сохранена и не конфликтует с A/AAAA.
- **Можно ли использовать `www.splash.nboard.au`?** Да. Добавьте второй поддомен в Railway и создайте отдельную CNAME-запись `www` → `splash-frontend-production.up.railway.app`, либо сделайте редирект с `www` на основное имя.
- **Нужно ли подключать корневой домен `nboard.au`?** Нет, если вы планируете открывать сайт только по `splash.nboard.au` и `recruitment.nboard.au`. Корневой домен подключают, когда нужен доступ без поддоменов (например, просто `https://nboard.au`).
...
Следуя этому чек-листу, сайт начнёт открываться по адресу `https://splash.nboard.au`, а Railway будет автоматически продлевать сертификат и обслуживать трафик.
