...
3. Railway-плагин Resend не требует «линковки» сервисов. Достаточно скопировать `RESEND_API_KEY` из плагина и добавить
   переменную в каждый нужный бэкенд.
4. После смены отправителя обновите переменную `RESEND_FROM` в бэкенде Railway и перезапустите деплой, чтобы приложение
...
5. Пока домен не подтверждён, Resend вернёт ошибку 400/403 с упоминанием домена, а API бэкенда ответит HTTP 424 с подсказкой
