...
export const InitiativeProfile = ({
  ...
  const handleSaveClick = async (closeAfterSave: boolean) => {
    if (!validateDraft()) {
      setBanner({ type: 'error', text: '«аполните об€зательные пол€.' });
      return;
    }
    if (!hasWorkstreams) {
      setBanner({ type: 'error', text: '—оздайте workstream, прежде чем добавл€ть инициативы.' });
      return;
    }
    setIsSaving(true);
    setBanner(null);
  ...
  const commentButtonLabel = isLoadingComments ? 'Loading commentsЕ' : `Comments${commentThreads.length ? ` (${commentThreads.length})` : ''}`;
